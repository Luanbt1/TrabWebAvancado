...
class assinantesController {
    async salvar(req, res) {
        ...
        if (assinante.nome == "" || assinante.sobrenome == "" || assinante.nascimento == "" || assinante.telefone == "" || assinante.endereco == ""
            || assinante.cidade == "" || assinante.estado == "") {
            res.status(400).json("Formato invalido!!");
        }
        else {
    ...
    async listar(req, res) {
        ...
        if (!(resultado && resultado.length > 0)) {
            res.status(404).json("O usuario nao foi encontrado!");
        } else {
